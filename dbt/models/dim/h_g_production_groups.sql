{{ config(
    ...
    schema='DDM',
    tags=['ch_hier','ddm'],
    order_by='g_production_groups_h, g_production_groups'
) }}
...
select toLowCardinality(h.cid) as g_production_groups_h, h.lid as g_production_groups,
    {% for i in range(1, 4) -%}
    path[{{ i }}] as  g_production_groups_{{ i }}{{ "," if not loop.last }}
    {% endfor %}
from h_all h
anti join (
    select ElementName from {{ref("stg_et_g_production_groups")}}
