...
{%- for i in range(0, 9) %}
  {%- if hier %}
    dictGet('DDM.d_g_production_groups_1', 'g_production_groups', 
      dictGetHierarchy('DDM.h_g_production_groups_1_{{ hier }}',xxHash32(g_production_groups))[{{(-1-i)}}])
  {%- elif i==0 %}
    g_production_groups
  {%- else %}
    ''
...
where 1 = 1 
{% if hier %} 
		AND xxHash32(g_production_groups) IN arrayPushBack(dictGetDescendants('DDM.h_g_production_groups_1_{{hier}}', xxHash32('{{node}}')), xxHash32('{{node}}'))
{% endif %};
